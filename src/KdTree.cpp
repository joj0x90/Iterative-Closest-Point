...
bool gs::KdTree::isLeaf() {
    if (__children[0] == nullptr && __children[1] == nullptr) return true;

    return false;
}

float gs::KdTree::split() { return __node->pos[__sortOn]; }
...
float gs::KdTree::nodeX() { return __node->pos[0]; }
float gs::KdTree::nodeY() { return __node->pos[1]; }
float gs::KdTree::nodeZ() { return __node->pos[2]; }

void gs::KdTree::radiusSearch(Point* p, float* radius, Point* result) {
    if (isLeaf()) {
        float d = sqrt(pow(__node->pos[0] - p->pos[0], 2.0) + pow(__node->pos[1] - p->pos[1], 2.0) +
                       pow(__node->pos[2] - p->pos[2], 2.0));
        if (d < *radius) {
            *radius = d;
...
void gs::KdTree::search(Point* p, Point* result) {
    ...
    float radius = sqrt(pow(p->pos[0] - result->pos[0], 2.0) + pow(p->pos[1] - result->pos[1], 2.0) +
                        pow(p->pos[2] - result->pos[2], 2.0));
