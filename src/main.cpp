...
void applyAffineTransform(std::vector<Point*>& points, float* rotationMatrix, float* translation) {
    ...
    for (int i = 0; i < points.size(); i++) {
        pRot.pos[0] = rotationMatrix[0] * points[i]->pos[0] + rotationMatrix[1] * points[i]->pos[1] +
                      rotationMatrix[2] * points[i]->pos[2] + translation[0];
        pRot.pos[1] = rotationMatrix[3] * points[i]->pos[0] + rotationMatrix[4] * points[i]->pos[1] +
                      rotationMatrix[5] * points[i]->pos[2] + translation[1];
        pRot.pos[2] = rotationMatrix[6] * points[i]->pos[0] + rotationMatrix[7] * points[i]->pos[1] +
                      rotationMatrix[8] * points[i]->pos[2] + translation[2];
...
void icpExample() {
    ...
    for (int i = 0; i < staticPointCloud.size(); i++) {
        printf("%0.2f, %0.2f, %0.2f \n", staticPointCloud[i]->pos[0], staticPointCloud[i]->pos[1],
               staticPointCloud[i]->pos[2]);
    }
    printf("\n");

    printf("Dynamic point Cloud: \n");
    for (int i = 0; i < dynamicPointCloud.size(); i++) {
        printf("%0.2f, %0.2f, %0.2f \n", dynamicPointCloud[i]->pos[0], dynamicPointCloud[i]->pos[1],
               dynamicPointCloud[i]->pos[2]);
    }
    printf("\n");
